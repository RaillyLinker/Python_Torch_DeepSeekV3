...
class MultiheadLatentAttention(nn.Module):
    def __init__(
        ...
        self.k_proj = nn.Linear(latent_dim, d_model, bias=False)
        self.v_proj = nn.Linear(latent_dim, d_model, bias=False)
    ...
    def forward(
        ...
        q = self.q_proj(latent).view(batch_size, seq_len, self.num_heads, self.head_dim)
        ...
        if causal_mask is not None:
            scores = scores + causal_mask  # add -inf where masked
